...
def check_website_changes():
	...
	cursor.execute('SELECT domain, ssdeep_hash, last_shown_time, hourly_ssdeep_hashes, last_hash_check_time FROM domains')
	results = cursor.fetchall()
	for domain, old_hash, last_shown_time, hourly_ssdeep_hashes, last_hash_check_time in results:
	...
	for domain in all_domains:
        ...
        if (all(okta in domain for okta in okta) or any(triggers in domain for triggers in triggers)) and not any(allowlist in domain for allowlist in allowlist):
			...
			if not result or (current_time - last_shown_time) >= time_window:
				...
				cursor.execute('''
				INSERT OR REPLACE INTO domains (domain, last_shown_time, ssdeep_hash, hourly_ssdeep_hashes, last_hash_check_time)
				VALUES (?, ?, ?, ?, ?)
				''', (domain, current_time, ssdeep_hash, hourly_ssdeep_hashes, current_time))
				conn.commit()
